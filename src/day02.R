...
##
## Date Created: Sat Dec  2 22:39:06 2023
##
...
input_df <- tibble(input = input_vec[-1])
input_df |> 
  rownames_to_column(var="ID") |> 
  separate_longer_delim(input, delim = "; ") |> 
  mutate(green = as.numeric(str_extract(str_extract(input, "[0-9]+ green"), "^[0-9]+")),
         red = as.numeric(str_extract(str_extract(input, "[0-9]+ red"), "^[0-9]+")),
         blue = as.numeric(str_extract(str_extract(input, "[0-9]+ blue"), "^[0-9]+"))
  ) |> 
  tidyr::replace_na(list(green = 0, red = 0, blue = 0)) |> 
  group_by(ID) |> 
  summarise(
    green = max(green),
    red = max(red),
...
input_df |> 
  ...
  group_by(ID) |> 
  summarise(
    green = max(green, na.rm=T),
    red = max(red, na.rm=T),
    blue = max(blue, na.rm=T)
  ) |>   
  mutate(
