...
##
## Date Created: Sun Dec  3 00:10:33 2023
##
...
input_df <- tibble(input = input_vec[[1]][-c(1,142)])

colnam <- rep(1, 140)
names(colnam) <- paste0(rep("col", 140), 1:140)

input_df_wide <- input_df |> 
  rownames_to_column(var = "row") |> 
  separate_wider_position(input, widths = colnam)

input_df <- input_df_wide |> 
  pivot_longer(cols = -row, names_to = "col") |> 
  mutate(
    row = as.numeric(row),
    col = as.numeric(str_extract(col, "[0-9]+"))
  )
...
# feels kinda cheating but check max length of number (i.e. max # of digits)
numbers <- input |> 
  str_extract_all("[0-9]+")

numbers[[1]] |> 
  as.numeric() |> 
  max() # 999 is max number

numberseeds <- input_df |> 
  semi_join(
    symbols,
    by = join_by(between(row, rowmin, rowmax),
                 between(col, colmin, colmax))
  ) |> 
...
# because maximum number of digits is 3, need to merge twice to 
# capture numbers that are 2 places away from seed
output1 <- input_df |> 
  filter(str_detect(value, "[0-9]")) |> 
  semi_join(
    numberseeds,
...
output1 |> 
  ...
  arrange(row, col) |> 
  mutate(
    value = if_else(is.na(value), valueblank, value)
  ) |> 
  select(-valueblank) |> 
  pivot_wider(
    names_from = col,
...
replace_na_list <- rep(".", 143)
names(replace_na_list) <- c("row", "row*", "col*", 1:140)


bind_rows(
  output2 |> 
    select(row, col.x, value.x, row.y.y, col.y.y) |> 
    rename_with(~c("row", "col", "value", "row*", "col*")) |> 
    distinct(),
  output2 |> 
    select(row, col.y, value.y, row.y.y, col.y.y) |> 
    rename_with(~c("row", "col", "value", "row*", "col*")) |> 
    distinct(),
  output2 |> 
    select(row, col.x.x, value.x.x, row.y.y, col.y.y) |> 
    rename_with(~c("row", "col", "value", "row*", "col*")) |> 
    distinct()
) |> 
  distinct() |> 
  ...
  replace_na(replace = as.list(replace_na_list)) |> 
  select(-row) |> 
  unite(
    col = "input",
    `1`:`140`,
  ...
  nest_by(`row*`, `col*`) |> 
  mutate(
    concat = str_flatten(data$input, "."),
    numlist = str_extract_all(concat, "[0-9]+")
  ) |> 
  unnest(numlist) |> 
  select(-data, -concat) |> 
  mutate(numlist = as.numeric(numlist)) |> 
  mutate(n = n()) |> 
  filter(n == 2) |> 
  ungroup() |> 
  nest_by(`row*`, `col*`) |> 
  mutate(
    gearratio = prod(data$numlist)
  ) |> 
  pull(gearratio) |> 
  sum()
